...
- Autosize finalist/frontier-confirmation proof: `test_llm_batch_profile_autosize_confirms_selected_candidate_by_default` covers the default final confirmation loop: a candidate that wins on the initial two seeds is re-profiled on fresh seed `104742` with `steps=2` and two runtime repeats under `confirm_seed_104742_steps_2_repeat_0/1`; when that confirmation shows instability, the robust score drops, selection switches to the stable candidate, and the loop confirms that new winner on seed `209471` before matrix launch. `test_llm_batch_profile_autosize_confirms_unselected_decision_frontier_challenger` covers the stronger C43 frontier rule: a non-selected challenger whose upper confidence overlaps the selected lower-confidence score is confirmed before matrix launch, and the report persists frontier margin fields. `test_llm_batch_profile_autosize_default_confirmation_rounds_cover_measured_frontier` covers the C44 budget rule: when `confirm_selected_max_rounds` is not explicit, the effective budget expands to the measured frontier size, confirms the winner plus two upper-confidence challengers with seeds `104742,209471,314200`, and leaves `confirmation_pending_shape_keys=()`. A short CUDA validation passed with `confirm_selected_max_rounds=4`, `measurement_seeds=[71,104800]`, `refinement_seeds=[209529]`, `confirmation_seeds=[314258]`, `confirmation_complete=true`, `confirmation_pending_shape_keys=[]`, `confirmation_decision_margin=210.160`, `measured_candidate_profile_count=12`, `refinement_profile_count=2`, `confirmation_profile_count=2`, selected `seq64_d64_h4_l2_b4_g2`, selected candidate `measurement_profile_count=6`, `measured_score_observation_count=4`, matrix throughput `304.437` tokens/s, GPU mean `16.000%`, power `40.043 W`, VRAM `982.769 MB`, `strict_extension_only_cases=1`, and `all_phases_active_cases=1`.
