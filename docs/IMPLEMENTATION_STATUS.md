...
- Run a more sustained latency/VRAM/energy matrix for the strict extension path, and add a WMMA/Tensor Core grad-weight variant only if the tiled kernel is not enough on larger LLM-shaped batches.
...
- Native ternary kernel validation: `tools\benchmark_ternary_kernel.py --native-backend extension --batch 128 --in-features 256 --out-features 256 --dtype fp16 --kernel-variant auto --autotune-warmup 1 --autotune-repeat 2 --warmup 5 --repeat 20` passed on RTX 5070 with extension v2, selected `warp_reduction_int2`, max error `0.000976`, packed weight compression `8x`, native `0.0350 ms`, full `BitLinear` forward `0.0976 ms`, PyTorch unpack+linear `0.3853 ms`, full forward speedup vs legacy native+STE dense `3.53x`, full forward+backward `0.6101 ms` vs dense STE legacy `0.9699 ms` (`1.59x`), `native_extension_grad_weight_dispatches=12`, and requantize/pack `0.1484 ms` vs PyTorch `0.5784 ms` (`3.90x`). A strict short matrix `tools\benchmark_ternary_kernel.py --matrix --dtype fp16 --kernel-variant auto --autotune-warmup 1 --autotune-repeat 2 --warmup 2 --repeat 8` passed on shapes `64x128x128`, `128x256x256`, `256x512x512` with `strict_extension_only=true`, min forward+backward speedup `1.37x`, average speedup `1.70x`, average `nvidia-smi` GPU utilization `8.67%` and average process CPU share `26.67%`; because the matrix is intentionally sub-second, the utilization samples are diagnostic, not a final occupancy study.
