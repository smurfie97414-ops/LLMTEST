...
- Autosize uncertainty-refinement proof: `test_llm_batch_profile_autosize_refines_uncertain_candidate_with_extra_seed` covers the new default refinement budget: after two measurement seeds, an uncertain high-upside candidate loses to a stable candidate by lower-confidence score; the harness then spends seed `104742` only on that uncertain candidate, recomputes mean/stddev/lower/upper confidence, and the final measured selection changes to the now-validated uncertain shape while reporting `refinement_rounds`, `refinement_profile_count`, `refinement_seeds` and before/after scores. A short CUDA validation with default refinement passed with `measurement_seeds=[71,104800]`, `refinement_seeds=[209529]`, 9 measured candidate profiles, refined shape `seq64_d96_h4_l2_b4_g2`, selected `seq64_d64_h4_l2_b4_g2`, robust score `3826.570`, mean `4508.380`, stddev `681.811`, matrix throughput `285.754` tokens/s, GPU mean `16.571%`, power `41.186 W`, VRAM `982.429 MB`, `strict_extension_only_cases=1`, and `all_phases_active_cases=1`.
