...
- Autosize finalist-confirmation proof: `test_llm_batch_profile_autosize_confirms_selected_candidate_by_default` covers the new default final confirmation step: a candidate that wins on the initial two seeds is re-profiled on fresh seed `104742` with `steps=2` and two runtime repeats under `confirm_seed_104742_steps_2_repeat_0/1`; when that confirmation shows instability, the robust score drops and selection switches to the stable candidate. A short CUDA validation passed with `measurement_seeds=[71,104800]`, `refinement_seeds=[209529]`, `confirmation_seeds=[314258]`, `measured_candidate_profile_count=12`, `refinement_profile_count=2`, `confirmation_profile_count=2`, selected `seq64_d64_h4_l2_b4_g2`, selected candidate `measurement_profile_count=6`, `measured_score_observation_count=4`, matrix throughput `323.779` tokens/s, GPU mean `13.750%`, power `41.912 W`, VRAM `987.000 MB`, `strict_extension_only_cases=1`, and `all_phases_active_cases=1`.
