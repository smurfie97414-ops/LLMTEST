...
- Cortex checkpoints persist and restore the phase controller's replay state, objective feedback state, future-contract ledger, retained ternary compression trace ledger, Bit/Skill/Causal/Uncertainty ledgers, cognitive memory, sleep pools and recursive-improvement archive summaries, so interrupted full-architecture training keeps the same P2-P4/P9/P10 and ledger audit context instead of resetting those modules while keeping trace memory bounded.
