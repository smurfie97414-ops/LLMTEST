...
- C4 large manifest preflight after full phase/input-anchor integration and RTX 5070 batch tuning: `tools\train_llm.py preflight-experiment experiments\c4_cuda_large_manifest.json --out-dir runs\cortex3-c4-cuda-large-preflight-batch4-current` passed with `batch_size=4`, `gradient_accumulation_steps=16`, estimated Cortex peak `4,768,726,245` bytes under `10,897,408,000` usable CUDA bytes, while preserving `2,097,152,000` planned train tokens, `65,536` tokens per optimizer step and `50,000,000` minimum corpus tokens.
