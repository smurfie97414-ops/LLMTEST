...
- Extend the current tiled/warp/requantize CuPy RawKernels toward lower-level CUDA/C++ packaging, reduce or fuse the remaining dense `grad_weight` STE cost, and run latency/VRAM/energy benchmarks across larger LLM-shaped batches.
...
- Native ternary kernel validation: `tools\benchmark_ternary_kernel.py --batch 128 --in-features 256 --out-features 256 --dtype fp16 --kernel-variant auto --autotune-warmup 1 --autotune-repeat 2 --warmup 3 --repeat 10 --autotune-cache runs\native_ternary_autotune_profile_short.json` passed on RTX 5070 with persisted profile, measured autotune candidates `tiled=0.1665 ms`, `warp=0.1368 ms`, selected `warp_reduction_int2`, max error `0.000976`, packed weight compression `8x`, native `0.1042 ms`, full `BitLinear` forward `0.1323 ms`, PyTorch unpack+linear `0.3461 ms`, legacy native+STE dense `0.3657 ms`, full-forward speedup vs legacy `2.76x`. After adding the packed-int2 CUDA `grad_input` backward kernel, the short `batch=128,in=256,out=256,fp16` benchmark measured `full_bitlinear_forward_backward_ms=0.9865`, `legacy_dense_ste_forward_backward_ms=1.3301`, speedup `1.35x`, with max error `0.000976`. After adding CUDA-fused requantize/pack, the same short benchmark measured `native_requantize_pack_ms=0.2245`, `torch_requantize_pack_ms=0.5901`, speedup `2.63x`. A short larger check `batch=512,in=512,out=512,fp16` measured `tiled=0.5668 ms`, `warp=0.3368 ms`, selected `warp_reduction_int2`, native `0.2561 ms`, PyTorch unpack+linear `0.2734 ms`, speedup `1.07x`.
- CUDA C++ extension toolchain audit: `tools\train_llm.py doctor --require-cuda --precision bf16 --device cuda` passed with `native_rawkernel_available=true`, `torch.version.cuda=12.8`, Visual Studio/cl detected and local `nvcc=13.2`. The stricter `--require-cuda-extension` mode fails intentionally on `cuda:extension_toolchain_ready` because PyTorch extension builds require an `nvcc` toolkit whose major.minor matches Torch CUDA; `nvidia-cuda-nvcc-cu12==12.8.93` was installed but its Windows wheel provides headers/NVVM/ptxas, not `nvcc.exe`.
