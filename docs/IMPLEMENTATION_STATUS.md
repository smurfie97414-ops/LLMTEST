...
- Autosize finalist/frontier-confirmation proof: `test_llm_batch_profile_autosize_blocks_unresolved_decision_after_winner_switch` covers the iterative confirmation loop: a candidate that wins on the initial two seeds is re-profiled on fresh seed `104742` with `steps=2` and two runtime repeats under `confirm_seed_104742_steps_2_repeat_0/1`; when that confirmation shows instability, the robust score drops, selection switches to the stable candidate, and the loop confirms that new winner on seed `209471`, then C45 blocks the still-unresolved decision margin before matrix launch. `test_llm_batch_profile_autosize_blocks_unresolved_confirmed_decision_frontier` covers the other C45 gate: a non-selected challenger is confirmed, `confirmation_pending_shape_keys=()`, but the decision margin remains non-positive, so the report fails with `selected_confirmation_decision_unresolved`, exposes `confirmation_decision_resolved=false`, and skips the matrix. `test_llm_batch_profile_autosize_uses_spare_round_to_resolve_confirmed_margin` covers C46: once pending confirmations are done, a spare `decision_margin_resolution` round profiles the winner and best challenger together, resolves the margin, and allows the matrix. `test_llm_batch_profile_autosize_default_confirmation_rounds_cover_measured_frontier` covers the C44/C45 positive path: when `confirm_selected_max_rounds` is not explicit, the effective budget expands to the measured frontier size, confirms the winner plus two upper-confidence challengers with seeds `104742,209471,314200`, leaves `confirmation_pending_shape_keys=()`, and requires `confirmation_decision_resolved=true`. A broad short CUDA validation consumed 20 measured profiles and 10 confirmation profiles across 4 confirmation rounds including 1 `decision_margin_resolution`, then correctly failed before matrix with `selected_confirmation_decision_unresolved`, `confirmation_complete=true`, `confirmation_decision_resolved=false`, and `confirmation_decision_margin=-6263.952`. A smaller CUDA validation passed with `confirm_selected_max_rounds=1`, `confirmation_seeds=[209529]`, `confirmation_complete=true`, `confirmation_decision_resolved=true`, `confirmation_decision_margin=616.691`, `measured_candidate_profile_count=4`, `confirmation_profile_count=2`, selected `seq64_d64_h4_l2_b4_g1`, matrix throughput `161.257` tokens/s, GPU mean `14.083%`, power `39.064 W`, VRAM `981.333 MB`, `strict_extension_only_cases=1`, and `all_phases_active_cases=1`.
