...
- `CortexTransformerLM` is a complete causal Transformer with tied embeddings, causal self-attention, MLP blocks, optional Cortex multi-horizon heads, an optional `BitLinear` ternary core for the Cortex model and a trainable skill-aware MoE path whose expert activations are recorded in the compression trace ledger.
...
- `CortexTrainingPhaseController` integrates P1-P10 into full LLM training when horizons are `[1, 2, 4, 8]` and skill-aware experts are enabled: verifier cycle, ternary forward traces, skill-expert activation traces, MTP/FSP contract ledger, cognitive memory reconstruction, certificate verification, causal attribution, minimal regrowth planning, fast/normal/careful inference, sleep replay batches and recursive-improvement gates.
...
- Cortex checkpoints persist and restore the phase controller's replay state, objective feedback state, future-contract ledger, retained ternary compression trace ledger, cognitive memory, sleep pools and recursive-improvement archive summaries, so interrupted full-architecture training keeps the same P2-P4/P9/P10 audit context instead of resetting those modules while keeping trace memory bounded.
