...
- Phase 2: compressed core instrumentation
  - Implemented baseline: PyTorch `BitLinear` sign+mask validated in `.venv` with `pip install -e .`, shared scales, zero-state lifecycle, residual synapse buffer, activation quantization logs, expert/KV/MTP/FSP trace ledger, real layer-forward events, packed int2 ternary buffers, CPU packed dispatch records, native CuPy RawKernel CUDA dispatch records, tiled/warp kernel variants and gradient-survival checks into inputs and weights through STE.
  - Next hardening: lower-level CUDA/C++ packaging, richer autotuning, then benchmark latency, VRAM and energy across larger LLM-shaped batches.
