...
- Phase 2: compressed core instrumentation
  - Implemented baseline: PyTorch `BitLinear` sign+mask validated in `.venv` with `pip install -e .`, shared scales, zero-state lifecycle, residual synapse buffer, activation quantization logs, expert/KV/MTP/FSP trace ledger, real layer-forward events, packed int2 ternary buffers, CPU packed dispatch records, native CuPy RawKernel CUDA dispatch records and gradient-survival checks into inputs and weights through STE.
  - Next hardening: replace the simple one-output-per-thread RawKernel with more aggressively tiled/fused CUDA kernels, then benchmark latency, VRAM and energy across larger LLM-shaped batches.
...
- Phase 4: cognitive memory and exact anchors
  - Implemented baseline: exact recent KV, compact latent old KV with Torch embeddings, Exact Anchor Ledger preservation, query-conditioned reconstruction, anchor fidelity verifier, trainable exact/latent/drop memory policy in the LLM forward, memory-policy loss, P4 supervision from decoded input anchors, memory-augmented inference generation for recovered anchors, audited replacement of weaker base answers and persisted memory compression/reconstruction fidelity reports.
  - Next: benchmark the learned memory policy on long-context held-out anchors, compare against deterministic memory-only ablations and promote anchor fidelity to a required cycle gate for long-context tasks.
