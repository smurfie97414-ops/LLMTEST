...
- Phase 2: compressed core instrumentation
  ...
  - Real-batch evidence: `tools/train_llm.py profile-batch` runs strict Cortex LLM training with optimizer/backward/requantize/P1-P10, extension-only kernel evidence, throughput, CPU/GPU/power/VRAM monitoring and CUDA memory snapshots.
  - Next hardening: broaden real Transformer shape coverage and repeat the latency, VRAM, energy, throughput and GPU occupancy profile under larger multi-shape and multi-seed LLM batches.
