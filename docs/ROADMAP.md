...
- Phase 2: compressed core instrumentation
  - Implemented baseline: PyTorch `BitLinear` sign+mask validated in `.venv` with `pip install -e .`, shared scales, zero-state lifecycle, residual synapse buffer, activation quantization logs, expert/KV/MTP/FSP trace ledger, real layer-forward events, packed int2 ternary buffers, CPU packed dispatch records, native RawKernel and strict C++/CUDA extension dispatch records, tiled/warp kernel variants, CUDA-event autotune with layer/global/persistent shape caches, custom fast STE autograd forward, CUDA fast STE `grad_input` kernel reading packed int2 weights, CUDA-fused requantize/pack after weight updates, versioned repack, CUDA 12.8 extension runtime doctor, LLM smoke with forced extension backend, and gradient-survival/parity checks into inputs and weights through STE.
  - Next hardening: make the extension backend the default hardened CUDA path, reduce or fuse dense `grad_weight` overhead, then benchmark latency, VRAM and energy across larger LLM-shaped batches.
