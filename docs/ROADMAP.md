...
- Phase 2: compressed core instrumentation
  - Implemented baseline: PyTorch `BitLinear` sign+mask validated in `.venv` with `pip install -e .`, shared scales, zero-state lifecycle, residual synapse buffer, activation quantization logs, expert/KV/MTP/FSP trace ledger, real layer-forward events, packed int2 ternary buffers, CPU/CUDA packed dispatch records and gradient-survival checks into inputs and weights through STE.
  - Next: replace the packed PyTorch unpack/matmul path with a fused custom CUDA kernel, then benchmark latency, VRAM and energy.
