...
Depuis C76/C84, cette politique memoire ne depend plus seulement de la difficulte locale des tokens: les reconstructions P4, les bindings Frontier et les inferences P8 qui selectionnent des segments retenus creent des credits d'utilite positifs, tandis que les segments learned-memory retenus mais non selectionnes par une reconstruction fidele creent des contre-credits negatifs. Ces credits normalisent un prior exact/latent/drop reinjecte dans `LearnedMemoryPolicy.forward` et dans `CortexObjective`, donc la memoire apprend progressivement quels choix de retention servent vraiment les phases aval et quels segments doivent etre compresses ou oublies.
