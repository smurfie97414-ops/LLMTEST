...
- `tests.test_cortex3` verifie maintenant que P1 rejette le code qui passe les exemples mais mute ses arguments, manque des tests caches ou devient non deterministe, et que les chaines entity-transfer changent bien de detenteur final sous anti-metamorphisme.
...
- `test_full_cortex_phase_controller_uses_all_modules_during_training` et `test_cortex_phase_state_survives_checkpoint_resume` verifient maintenant que les patchs modele P7/P10 signent `protected_replay_source=phase_balanced_replay`, couvrent les phases disponibles via `protected_replay_phase_ids`, exposent `regrowth_model_protected_replay_phase_count` / `recursive_model_protected_replay_phase_count`, et restaurent `replay_phase_ids` depuis checkpoint.
...
Depuis C76/C84/C98, cette politique memoire ne depend plus seulement de la difficulte locale des tokens: les reconstructions P4, les bindings Frontier et les inferences P8 qui selectionnent des segments retenus creent des credits d'utilite positifs, tandis que les segments learned-memory retenus mais non selectionnes par une reconstruction fidele creent des contre-credits negatifs. Ces credits normalisent un prior exact/latent/drop reinjecte dans `LearnedMemoryPolicy.forward` et dans `CortexObjective`, puis biaisent aussi le ranking de `CognitiveMemory.reconstruct`; la memoire apprend donc quels choix de retention servent vraiment les phases aval, quels segments doivent etre compresses ou oublies, et quels segments doivent etre preferes lors d'une reconstruction future.
...
Depuis C76/C84/C98, les reconstructions fideles creent aussi des credits d'utilite lies aux decisions appliquees. Les segments selectionnes apportent un credit positif; les segments learned-memory retenus mais non selectionnes apportent un contre-credit qui pousse le prior vers latent/drop au lieu de renforcer exact/latent. Les credits issus de `learned_memory_policy` mettent a jour un prior exact/latent/drop persistant; ce prior biaise les logits de `LearnedMemoryPolicy`, ajoute une cible d'alignement dans `CortexObjective` et sert maintenant de biais direct dans le ranking exact/latent de `CognitiveMemory.reconstruct`. Les exemples d'ancrage restent du replay et les echecs de fidelite peuvent faire echouer l'audit, mais la partie apprise regarde maintenant ce qui a vraiment ete reutilise par P4/P8/Frontier, ce qui n'avait pas besoin d'etre retenu, puis influence la prochaine reconstruction sans attendre uniquement un nouveau gradient.
