...
Le doctor distingue maintenant le backend RawKernel actuel et le packaging C++/CUDA. Sur ce PC, `tools/train_llm.py doctor --require-cuda --require-cuda-extension --precision bf16 --device cuda` passe avec `native_rawkernel_available=true`, `nvcc=12.8` depuis `C:\Users\hight\.codex\cuda-12.8\Library` et Visual Studio Build Tools 2022. Le smoke `tools\run_cuda128_extension_smoke.ps1` compile une vraie extension PyTorch CUDA et exécute un kernel sur le RTX 5070. Le backend training Cortex utilise encore les RawKernels CuPy pour les kernels ternaires; la prochaine étape est la migration des kernels Cortex eux-mêmes vers ce packaging extension.
