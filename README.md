...
- `cortex3_frontier.py` : découverte de compétences frontières avec slow-solve vérifié, compilation en micro-circuit `BitLinear`, registre runtime de circuits DSV-passing, sélection par skill/invariants via `CompiledFrontierAgent`, persistance `frontier_registry.json` + checkpoints, branchement P8 via `UltraFastInferenceEngine` et preuve d'usage FastSolve compilé dans l'audit LLM ;
