...
`requirements-cuda-cu128.txt` installe aussi `cupy-cuda12x` et `ml_dtypes`, nécessaires au backend diagnostic RawKernel. Le backend training peut maintenant être forcé sur la vraie extension PyTorch C++/CUDA avec `--native-ternary-backend extension`; sur ce PC, les libs CUDA 12.8 dev locales (`cuda-libraries-dev` et `cuda-cccl` dans `C:\Users\hight\.codex\cuda-12.8\Library`) fournissent `nvcc`, `cusparse.h` et `nv/target`. En mode `auto`, `BitLinear` mesure `tiled` et `warp` sur la shape CUDA courante, cache le meilleur choix par device/dtype/shape, peut sauvegarder/recharger un profil JSON via `--autotune-cache`, saute le `F.linear` dense STE dans le forward grâce à un autograd custom, calcule `grad_input` CUDA depuis les poids int2 packés dans le backward fast STE, requantize/repack les poids en CUDA fusionné après update, et ne repack les poids que si leur version change. Sur RTX 5070, les benchmarks courts locaux donnent :
...
- `batch=128, in=256, out=256, fp16`, forward+backward : fast STE natif `0.9865 ms` contre dense STE legacy `1.3301 ms`, soit `1.35x`; le `grad_input` lit les codes int2 packés, tandis que `grad_weight` reste dense/exact.
...
2. Durcir Phase 2 au-delà du backend extension actuel : réduire ou fusionner le coût `grad_weight`, figer le backend extension comme chemin par défaut, puis benchmarker latence/VRAM/énergie sous vrais batchs LLM.
